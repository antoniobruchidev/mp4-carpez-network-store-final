...
def dashboard(request):
    if request.user.is_superuser:
        ...
        page_display = "{} to {} of {} entries".format(
            int(page - 1) * 20 + 1, int(page) * 20, paginated_orders.count
        )
        if page == paginated_orders.num_pages:
            page_display = "{} to {} of {} entries".format(
                int(page - 1) * 20 + 1, paginated_orders.count, paginated_orders.count
            )
        page_count = []
        for item in range(0, paginated_orders.num_pages):
            page_count.append(item + 1)
        reviews = Review.objects.exclude(rating=None).filter(store_answer=None)
        ...
        if "tag" in request.POST and "friendly_tag" in request.POST:
            try:
                ...
                if "unique constraint" in error.__str__():
                    data["error"] = (
                        "Attempt to add tag violates unique \
    ...
    if request.user.is_superuser:
        ...
        if "name" in request.POST and "friendly_name" in request.POST:
            try:
                ...
                if "unique constraint" in error.__str__():
                    data["error"] = (
                        "Attempt to add category violates \
    ...
    if request.user.is_superuser:
        ...
        if "brand" in request.POST and "support_page" in request.POST:
            try:
                ...
                if "unique constraint" in error.__str__():
                    data["error"] = (
                        "Attempt to add brand violates unique \
...
def delete_tag(request, tag_id):
    if request.user.is_superuser:
        ...
        try:
            ...
            if tag.tag == "desktop" or tag.tag == "laptop" or tag.tag == "clearance":
                data["error"] = (
                    "Desktop, Laptop and Clearance tags are \
                     required. Aborted."
                )
                status = 403
            else:
